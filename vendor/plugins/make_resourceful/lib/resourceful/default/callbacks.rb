  ...
  module Default
    ...
    module Callbacks
      ...
      def response_for(event)
		...
		logger.info("response callback: #{key}:#{value}")
	...
	logger.info("firing callback for #{type}:#{name}")
	logger.info("block is: #{self.class.read_inheritable_attribute(:resourceful_callbacks)[type][name]}")
