...
module Squirrel
  ...
  module CalculationsHook

    def calculate_with_squirrel *args, &blk
      if blk
        query = Query.new(self, &blk)
        query.execute_calculate(*args)
      else
        calculate_without_squirrel(*args)
      end
    end

    def self.included base
      if ! base.instance_methods.include?('calculate_without_squirrel') &&
           base.instance_methods.include?('calculate')
        base.module_eval do
          alias_method :calculate_without_squirrel, :calculate
          alias_method :calculate, :calculate_with_squirrel
        end
      end
    end
  end
  ...
  class Query
    ...
    # Runs the block which builds the conditions and then performs the specified calculation
    # on them (all the ones valid for ActiveRecord:Calculations::ClassMethods. Since AR funnels
    # all of the class methods through ::calculate, this should be an invisible addition to AR.
    def execute_calculate *args
      opts = args.last.is_a?(Hash) ? args.last : {}
      model.send(:with_scope, :find => opts) do
        model.calculate args[0], args[1], to_find_parameters
      end
    end
    ...
    def to_find_parameters
      ...
      find_parameters[:group     ] = to_find_group      unless to_find_group.blank?
    ...
    # Delegates the to_find_group call to the root ConditionGroup
    def to_find_group
      @conditions.to_find_group
    end
    ...
    class ConditionGroup
      ...
      def to_find_group
        if @group.blank?
          ...
          @group.collect do |col|
            col.respond_to?(:full_name) ? col.full_name : col
          end.join(", ")
        end
      end
